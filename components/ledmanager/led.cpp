...
Led::Led(gpio_num_t gpio) {
    ...
    ESP_ERROR_CHECK(rmt_new_tx_channel(&channel_config, &channel_));
    ...
    ESP_ERROR_CHECK(rmt_new_bytes_encoder(&encoder_config, &encoder_));
...
void Led::on(uint8_t red, uint8_t green, uint8_t blue) {
    ...
    ESP_ERROR_CHECK(rmt_transmit(channel_, encoder_, colour, sizeof(colour),
                                 &transmit_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(channel_, -1));
}
